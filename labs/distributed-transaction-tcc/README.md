...
도메인은 앞 실험실과 같다. 좌석은 `seat_db`에, 지갑은 `wallet_db`에 있고 서로 다른 MySQL 서버다. **XA는 쓰지 않는다.** 각 참여자가 자기 로컬 트랜잭션만 쓴다.

## 확정된 것과 잡아 둔 것을 따로 센다
...
**Try는 `held_count`와 `frozen`을 올리고 커밋한다.** Confirm은 그걸 `reserved_count`로 옮기고, Cancel은 도로 내린다.
...
검사도 변경도 엔티티가 한다. 참여자는 `seat.hold(2)`를 부르고 결과만 본다. **잡아 둔 좌석을 정원에서 함께 빼는 이 뺄셈이 곧 락을 대신하는 것이다.**
...
여기가 2PC와 결정적으로 갈린다. **2PC의 롤백은 데이터베이스가 해 줬지만, TCC의 Cancel은 참여자가 직접 짜는 업무 로직이다.** 잡아 둔 만큼 빼는 UPDATE를 한 번 더 실행할 뿐이다.
...
<mark>예약 1의 로컬 트랜잭션은 Try에서 이미 커밋됐으니 쥐고 있는 락 자체가 없다.</mark> 코디네이터가 늦든 죽든 다른 요청은 영향받지 않는다. 2PC가 못 했던 게 바로 이것이다.

대신 읽는 쪽은 확정되지 않은 값을 보게 된다. **격리를 포기한 것이다.** 2PC에서는 커밋 전 값이 아예 안 보였다.

## 락이 사라진 자리에 생긴 일

락은 사실 일을 하고 있었다. 같은 요청이 두 번 오거나 순서가 뒤바뀌어도 데이터베이스가 뒤에서 막아 줬다. 그게 없어졌으니 **참여자가 직접 메워야 한다.**
...
마지막 두 줄이 한 쌍이다. Try 요청이 유실되거나 늦어져도 코디네이터는 취소를 결정하고 Cancel을 보낸다. 참여자에게는 본 적 없는 요청의 Cancel이다. **되돌릴 게 없다고 그냥 넘기면, 뒤늦게 도착한 Try가 잡아 둔 좌석을 아무도 회수하지 않는다.** 그래서 빈 취소는 무시가 아니라 기록이어야 한다.
...
같은 걸 만드는 방법이 여럿이었다. **고른 이유보다 버린 이유가 나중에 더 필요하다.**

| 갈림길 | 고른 것 | 버리고 그 이유 |
| --- | --- | --- |
| 코디네이터 | 테스트 메서드의 제어 흐름 | **Saga 오케스트레이터 프레임워크** — 상태 전이 자체를 보여주는 게 목적이라 프레임워크가 전이를 감추면 정반대다 |
| Try/Confirm/Cancel 호출 | 참여자 빈의 메서드를 코디네이터가 직접 호출 | **메시지 큐 기반 비동기 디스패치** — 유실·재시도까지 재현하려면 큐와 컨슈머까지 만들어야 하는데, 이 실험실은 락 없이 정합성을 지키는 로직만 보이면 된다 |
...
| 영속성 계층 | Spring Data JPA. DataSource·EntityManagerFactory·TransactionManager를 좌석/지갑 두 벌로 나눠 둔다 | **단일 트랜잭션 매니저** — 한 매니저가 두 DB를 아우르면 커밋이 갈라지는 지점 자체가 사라져 실험이 성립하지 않는다 |
| 자원 수량 변경 | 엔티티를 읽어 메서드를 부르고 저장. 정원·잔액 검사도 엔티티 안에 있다 | **벌크 UPDATE(`@Modifying`)의 WHERE에 검사를 넣기** — 동시 요청에는 더 강하지만 리포지토리가 SQL 덩어리가 된다. 이 실험실은 순차 실행이라 얻는 것보다 읽기 어려워지는 쪽이 컸다 |
| 동시성 제어 | 없다. 요청 상태 판단은 로그 한 줄을 읽어서 한다 | **비관적 락·낙관적 락** — Confirm/Cancel 중복과 빈 취소는 순서 문제이지 동시성 문제가 아니라 락 없이도 재현된다. 동시 요청은 아래 "다루지 않은 것"으로 남겼다 |
...
애플리케이션 코드는 `@TwoPhaseBusinessAction`으로 Try/Confirm/Cancel 메서드만 선언하면 되고, 위 표의 왼쪽 절반(로그 관리·멱등·빈 취소·매달림 판단)을 프레임워크가 대신한다. **이 실험실과 달리 Seata는 Try 요청이 코디네이터에게 도착하지 않아도, 코디네이터가 Cancel을 보내면 그걸 감지하고 자동으로 빈 취소를 기록한다.**
...
- **동시 요청.** 테스트는 순차 실행이고 락을 하나도 걸지 않았다. 그래서 서로 다른 요청이 같은 좌석을 동시에 잡으면 갱신 유실이 난다. 둘 다 `held_count = 0`을 읽고 각자 2를 더해 저장하면 4가 아니라 2가 남는다. 실무에서는 `@Version`을 붙여 낙관적 락을 걸고 실패 시 재시도하거나, 검사를 UPDATE의 WHERE로 내려 한 문장에서 끝낸다. **이건 TCC의 문제가 아니라 읽고-고쳐-쓰기의 문제라 별개로 다뤄야 한다**
...
참여자는 `seat`·`wallet` 두 패키지로 나뉘고, 각 패키지의 설정 클래스가 자기 DataSource·EntityManagerFactory·TransactionManager를 한 벌씩 만든다. `@Transactional`에 어느 트랜잭션 매니저를 쓸지 이름으로 지정하는 것이 그래서다.

<mark>테스트 클래스에는 `@Transactional`을 붙이지 않는다.</mark> 붙이면 테스트 메서드 하나가 통째로 한 트랜잭션이 되어, 참여자가 단계마다 커밋한다는 전제가 무너진다. 검증 메서드가 매번 새 트랜잭션으로 읽어야 "이미 커밋되어 남에게도 보이는 값"을 확인할 수 있다.
