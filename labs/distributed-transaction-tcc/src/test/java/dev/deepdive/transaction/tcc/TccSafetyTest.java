 ...
 */
class TccSafetyTest extends TccJpaTest {

    @BeforeEach
    void seedDatabases() {
        // 좌석 10석, reservedCount = 0, heldCount = 0. 로그도 비어 있다.
        seed(100_000);
    }

    @Test
    void Confirm을두번받아도_한번만적용된다() {
    ...
    @Test
    void Cancel을두번받아도_한번만적용된다() {
        assertThat(seatParticipant.tryHold(REQUEST_ID, WORKSHOP_ID, SEAT_COUNT)).isTrue();

        // Cancel도 똑같이 재시도된다. 되돌리기는 뺄셈이라 두 번 적용되면 heldCount가 -2로 내려가고,
